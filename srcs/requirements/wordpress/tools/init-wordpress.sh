...
if ! $(su -s /bin/sh -c "wp core is-installed --path=${WP_PATH}" www-data); then
	# Install WordPress
	su -s /bin/sh -c "wp core install --url=${DOMAIN_NAME} --title="Gamoreno-website" --admin_user="Symply-God" --admin_password=${GNRL_ROOT_PASSWORD} --admin_email="gamoreno@student.42.fr" --path=${WP_PATH}" www-data
	# Create a second user
	su -s /bin/sh -c "wp user create gabo gabo@${DOMAIN_NAME} --role=author --user_pass=${WP_PASSWORD} --path=${WP_PATH}" www-data
fi
